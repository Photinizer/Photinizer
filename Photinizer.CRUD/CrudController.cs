...
public abstract class CrudController<T, TId, TFilter>(IMessenger messenger)
{
    ...
    protected void AddCreate(Func<T, Task<TId>> onCreate)
        => messenger.OnQueryAsync<T>(Endpoint("create"), async entity => await onCreate(entity));

    protected void AddReadAll(Func<TFilter, Task<IReadOnlyCollection<T>>> onReadAll)
        => messenger.OnQueryAsync<TFilter>(Endpoint("readAll"), async filter => await onReadAll(filter));

    protected void AddRead(Func<TId, Task<T>> onRead)
        => messenger.OnQueryAsync<TId>(Endpoint("read"), async id => await onRead(id));

    protected void AddUpdate(Func<T, Task> update)
        => messenger.OnTaskAsync<T>(Endpoint("update"), async entity => await update(entity));

    protected void AddDelete(Func<TId, Task> delete)
        => messenger.OnTaskAsync<TId>(Endpoint("delete"), async id => await delete(id));
